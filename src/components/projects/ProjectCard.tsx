...
export function ProjectCard({ title, category, image, color = '#e5e5e5' }: ProjectCardProps) {
  return (
    <motion.div
    ...
    >
      ...
      >
        ...
        {image ? (
            <div className="relative w-full h-full"> 
                {/* Use Next.js Image if real URL, otherwise styled div */}
                 {/* For template purposes, we use colored background mostly, but if image valid: */}
            </div>
        ) : (
      ...
      <div className="absolute bottom-0 left-0 w-full p-6 text-[#F5F3EB] translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
        <div className="overflow-hidden">
            <h3 className="text-2xl font-bold mb-1 transform translate-y-8 group-hover:translate-y-0 transition-transform duration-500 delay-75">{title}</h3>
        </div>
        <div className="overflow-hidden">
            <p className="text-sm font-medium uppercase tracking-wider opacity-0 group-hover:opacity-100 transform translate-y-8 group-hover:translate-y-0 transition-all duration-500 delay-150">{category}</p>
        </div>
      </div>
